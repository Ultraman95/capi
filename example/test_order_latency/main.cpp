  ...
  void processEvent(const Event& event, Session* sessionPtr) override {
    if (event.getType() == Event::Type::SUBSCRIPTION_STATUS) {
      ...
      if (message.getType() == Message::Type::SUBSCRIPTION_STARTED) {
        Request request(Request::Operation::GET_OPEN_ORDERS, "bybit", this->symbol);
      ...
      if (message.getType() == Message::Type::GET_OPEN_ORDERS) {
        for (int i = 0; i < this->numOrders; ++i) {
          sessionPtr->setTimer(
              ...
              [this, sessionPtr]() {
                Request request(Request::Operation::CREATE_ORDER, "bybit", this->symbol);
      ...
      if (message.getType() == Message::Type::EXECUTION_MANAGEMENT_EVENTS_ORDER_UPDATE) {
        ...
        if (status == "live") {
          ...
          Request request(Request::Operation::CANCEL_ORDER, "bybit", this->symbol);
...
int main(int argc, char** argv) {
  ...
  const auto& bybit_rest_fast_url = ccapi::UtilSystem::getEnvAsString("BYBIT_REST_FAST_URL");
  const auto& bybit_websocket_fast_url = ccapi::UtilSystem::getEnvAsString("BYBIT_WEBSOCKET_FAST_URL");
  auto url_rest_base = sessionConfigs.getUrlRestBase();
  if (!bybit_rest_fast_url.empty()) {
    url_rest_base.at("bybit") = bybit_rest_fast_url;
  }
  ...
  if (!bybit_websocket_fast_url.empty()) {
    url_websocket_base.at("bybit") = bybit_websocket_fast_url;
  }
  ...
  Subscription subscription("bybit", symbol, "ORDER_UPDATE", "", websocketOrderEntrySubscriptionCorrelationId);
